...
const main = async () => {
	...
	await run("git push");
	await run("pnpm --filter @url122/astro build");
	if (values.otp) {
	...
	await run(
		`gh release create ${tag} --title ${tag} --notes "Please refer to [CHANGELOG.md](https://github.com/altie122/URL122AstroIntergration/blob/main/package/CHANGELOG.md) for details."`,
	);
};
